...
class fr5robot(facility):
    ...
    def __init__(self,name:str,type:str,ip:str):
        super().__init__(name,type)
        self.robot = Robot.RPC(ip)
        self.start_pos_change = [0,0,0,0,0,0]
        self.message_start()
    ...
    def cmd(self,command,params):
        if command == "moveto":
            ...
            self.MoveTo(x=args[], y=0, z=0, r1=0, r2=0, r3=0,type = "MoveL",vel=default_speed,acc=default_acc)
    ...
    def MoveTo(self,x=0, y=0, z=0, r1=0, r2=0, r3=0,type = "MoveL",vel=default_speed,acc=default_acc):
        new_list = [0.0 + val + self.start_pos_change[i] for i , val in enumerate([x, y, z, r1, r2, r3])]
        new_pose = tuple(new_list)
        self.message_head()
        print("新位姿",new_pose)
        self.Move(new_pose,type,vel,acc)
